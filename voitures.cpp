...
bool voitures::ajouter()
{
    ...
    QString tarifsloca_string=QString::number(tarifsloca);
     query.prepare("INSERT INTO VOITURES(MARQUE,MODELE,AFB,PLAQUE_DIMMAT,KILOMETRAGE,ETAT,TARIFSLOCA)"
                   " VALUES ( :marque,:modele, :afb, :plaque_dimmat, :kilometrage, :etat, :tarifsloca)");
...
{
    QSqlQuery query;
    query.prepare("UPDATE VOITURES SET marque= :marque,modele= :modele,afb= :afb,plaque_dimmat= :plaque_dimmat,kilometrage= :kilometrage,etat= :etat,tarifsloca= :tarifsloca WHERE plaque_dimmat= :plaque_dimmat ");
    QString afb_string=QString::number(afb);
    QString kilometrage_string=QString::number(kilometrage);
    QString tarifsloca_string=QString::number(tarifsloca);
     query.bindValue(":marque",marque);
...
model->setQuery("SELECT * FROM VOITURES ORDER BY AFB");
model->setHeaderData(0,Qt::Horizontal,QObject::tr("MARQUE"));
model->setHeaderData(1,Qt::Horizontal,QObject::tr("MODELE"));
model->setHeaderData(2,Qt::Horizontal,QObject::tr("AFB"));
model->setHeaderData(3,Qt::Horizontal,QObject::tr("PLAQUE_DIMMAT"));
...
model->setQuery("SELECT * FROM VOITURES ORDER BY TARIFSLOCA");
model->setHeaderData(0,Qt::Horizontal,QObject::tr("MARQUE"));
model->setHeaderData(1,Qt::Horizontal,QObject::tr("MODELE"));
model->setHeaderData(2,Qt::Horizontal,QObject::tr("AFB"));
model->setHeaderData(3,Qt::Horizontal,QObject::tr("PLAQUE_DIMMAT"));
