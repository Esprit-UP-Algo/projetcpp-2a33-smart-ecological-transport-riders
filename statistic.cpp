...
QSqlQuery qry;
int i=0;
qry.exec("SELECT CONTRATID FROM GESTIONCONTRAT");
while (qry.next())
{
    QString CONTRATID = qry.value(0).toString();
    i++;
    *ticks<<i;
    *labels <<CONTRATID;
}
...
QLinearGradient gradient(0, 0, 0, 400);
                  ...
                  amande->setStackingGap(1);
                   //couleurs
                  amande->setName("Repartition des contrats selon montant ");
                  ...
                  QSqlQuery q1("SELECT MONTANT FROM GESTIONCONTRAT");
                  while (q1.next()) {
